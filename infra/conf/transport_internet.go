...
func (p TransportProtocol) Build() (string, error) {
	...
	case "xhttp", "splithttp":
		return "", errors.PrintRemovedFeatureError("XHTTP transport", "RAW (TCP) transport")
	case "kcp", "mkcp":
		return "", errors.PrintRemovedFeatureError("mKCP transport", "RAW (TCP) transport")
	case "grpc":
		return "", errors.PrintRemovedFeatureError("gRPC transport", "RAW (TCP) transport")
	case "ws", "websocket":
		return "", errors.PrintRemovedFeatureError("WebSocket transport", "RAW (TCP) transport")
	case "httpupgrade":
		return "", errors.PrintRemovedFeatureError("HTTPUpgrade transport", "RAW (TCP) transport")
	case "h2", "h3", "http":
		return "", errors.PrintRemovedFeatureError("HTTP transport (without header padding, etc.)", "XHTTP stream-one H2 & H3")
	case "quic":
		return "", errors.PrintRemovedFeatureError("QUIC transport (without web service, etc.)", "XHTTP stream-one H3")
	case "hysteria":
		return "", errors.PrintRemovedFeatureError("Hysteria transport", "RAW (TCP) transport")
	default:
...
func (c *StreamConfig) Build() (*internet.StreamConfig, error) {
	...
	case "reality":
		if config.ProtocolName != "tcp" {
			return nil, errors.New("REALITY only supports RAW (TCP) for now.")
		}
